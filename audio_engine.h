...
// Suspend / resume the Core-0 audio task. Use when an app wants the I²S +
// codec idle (so it doesn't contend with touch/QSPI on long-text screens).
// Safe to call resume() when the task isn't suspended.
void audio_engine_suspend();
void audio_engine_resume();
